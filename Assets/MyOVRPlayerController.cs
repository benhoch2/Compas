...
public class MyOVRPlayerController : MonoBehaviour
{
	...
	/// <summary>
	/// If true, each MyOVRPlayerController will use the player's physical height.
	/// </summary>
	...
	/// with the MyOVRPlayerController.
	/// </summary>
    ...
    private float rightRocket = 0.0f;
    private float leftRocket = 0.0f;
	...
	void Awake()
	{
		...
		if (Controller == null)
			Debug.LogWarning("MyOVRPlayerController: No CharacterController attached.");
		...
		if (CameraRigs.Length == 0)
			Debug.LogWarning("MyOVRPlayerController: No OVRCameraRig attached.");
		else if (CameraRigs.Length > 1)
			Debug.LogWarning("MyOVRPlayerController: More then 1 OVRCameraRig attached.");
		else
	...
	void Update()
	{
        ...
        rightRocket = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, OVRInput.Controller.RTouch);
        leftRocket = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, OVRInput.Controller.LTouch);


        float upForce = (getControllerDirection(OVRInput.Controller.RTrackedRemote).y * rightRocket 
                        + getControllerDirection(OVRInput.Controller.LTrackedRemote).y * leftRocket);
        isFlyingUp = upForce > 0;
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
            ...
            if (rightRocket != 0.0f)
            {
                Vector3 rightDir = getControllerDirection(OVRInput.Controller.RTrackedRemote);
                
                MoveThrottle += rightDir * rightRocket * ThrustForce;
            }
            if (leftRocket != 0.0f)
            {
                Vector3 leftDir = getControllerDirection(OVRInput.Controller.LTrackedRemote);

                MoveThrottle += leftDir * leftRocket * ThrustForce;
            }
    ...
    private Vector3 getControllerDirection(OVRInput.Controller controllerType)
    {
        Quaternion q = OVRInput.GetLocalControllerRotation(controllerType);

        Transform root = CameraRig.trackingSpace;

        return q * root.forward;
    }
}
